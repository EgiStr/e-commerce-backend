...
class Image(models.Model):
    ...
    varian = models.ForeignKey("Varian", related_name="image_varian", on_delete=models.CASCADE,blank=True, null=True)
...
class Varian(models.Model):
    ...
    price = models.IntegerField(_(" price in IDR"),validators=[MaxValueValidator(99999999),MinValueValidator(0)])

    def __str__(self) -> str:
        return f"varian {self.product.title} name {self.name}"

    def get_image_varian(self):
        return self.image_varian.all()
...
class Rating(models.Model):
    ...
    rating  = models.PositiveIntegerField(_(" rating"),validators=[MinValueValidator(1),MaxValueValidator(10)])
...
class Bookmark(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL,related_name="bookmark" ,on_delete=models.CASCADE)
